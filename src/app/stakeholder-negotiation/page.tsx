...
export default function PoliticalAssemblyDebatePage() {
  ...
  // Track when agents are speaking to disable policy selection and completion
  ...
  const startNewPolicyDebate = (policyId: string) => {
    ...
    // IMPORTANT: We're no longer filtering out previous policy logs
    // This preserves the full conversation history for the PDF generation
    // and summary that will be generated by Groq later
    
    // Add a policy transition message to the logs
    const transitionMessage = {
      agent: 'System',
      content: `--- Moving to discuss ${newPolicy.title} policy ---`,
      timestamp: new Date().toISOString(),
      emotion: 'neutral',
      isUser: false,
      policyAreaId: policyId,
      id: `transition-${Date.now()}`
    };
    setNegotiationLogs([...negotiationLogs, transitionMessage]);
          ...
          {selectedPolicies.map(policy => {
            ...
            return (
              <button
                ...
                } shadow hover:${canSelect && !isAgentSpeaking ? 'bg-blue-200' : 'bg-gray-100'} relative ${
                  isAgentSpeaking ? 'opacity-50 cursor-not-allowed' : ''
                }`}
                onClick={() => {
                  if (canSelect && !isAgentSpeaking) {
                ...
                disabled={(isConversationStarted && !currentPolicyDebateComplete && !isActive) || isAgentSpeaking}
              >
                {policy.title}
                {isActive && isConversationStarted && !currentPolicyDebateComplete && (
            ...
            {!isConversationStarted ? (
              ...
              <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
                ...
                <ConversationManager
                  ...
                  onSpeakingStateChange={setIsAgentSpeaking}
